...
export const createStatusStr = ({ category, type, action, verb, status, message, context }: ILogContext = {}) => {
  ...
  switch (context) {
    ...
    case 'server': {
      const color = str.includes('error') ? '\x1b[1;41m ' : str.includes('idle') ? '\x1b[1;43m' : '\x1b[1;42m';
...
export const fluxLog = ({ type, action, verb, status, message }: ILogContext) => {
